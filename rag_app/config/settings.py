...
def ensure_weaviate_schema_http() -> None:
    ...
    try:
        # IMPORTANT: use json= to send application/json
        r = requests.post(f"{WEAVIATE_URL}/v1/schema/classes", json=payload, timeout=20)
        r.raise_for_status()
...
def weaviate_batch_upsert(objects: List[Dict[str, Any]]) -> None:
    ...
    # IMPORTANT: use json= to force application/json
    r = requests.post(f"{WEAVIATE_URL}/v1/batch/objects", json=payload, timeout=60)
...
def weaviate_graphql(query: str, variables: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # IMPORTANT: use json= to force application/json
    r = requests.post(f"{WEAVIATE_URL}/v1/graphql", json=body, timeout=30)
    if r.status_code >= 300:
